...
def write_ply_mesh(ply_path, mesh_name, mesh, ffaces_mats):
    ...
    if len(uv_layers) > 0:
        if uv_layers.active and uv_layers.active.data:
	...
	#save ffaces_mats - mesh data for only 1 selected material index
    for face in ffaces_mats:#mesh.loop_triangles: #ffaces_mats:
        fvi = []
        ...
        for j, vertex in enumerate(face.vertices):
            ...
            if uv_layer:
                ...
                # Flip UV Y axis. Blender UV coord is bottom-left, Mitsuba is top-left. PBRT4-not sure???
                #work correct:
                uv_coord = (uv_layer[loop_index].uv[0], 1.0 - uv_layer[loop_index].uv[1])
            ...
            if face.use_smooth:
                ...
                if uv_layer:
                    vert_data = (v.co[:], face.normal[:], uv_layer[loop_index].uv[:])
