...
"""
def dict2xml(d, root_node=None, curOffset = "", id = -1):
    ...
    if isinstance(d, dict):
        ...
        for key, value in dict.items(d):
            if isinstance(value, dict):
                children.append(dict2xml(value, key, curOffset, id))
        ...
        for value in d:
            children.append(dict2xml(value, root_singular, curOffset, id))

    end_tag = '>'+"\n" if 0 < len(children) else '/>'+"\n"

    if wrap or isinstance(d, dict):
        xml = curOffset + '<' + root + xml + end_tag
